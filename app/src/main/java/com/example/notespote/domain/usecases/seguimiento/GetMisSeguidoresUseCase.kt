// domain/usecases/seguimiento/GetMisSeguidoresUseCase.kt
...
class GetMisSeguidoresUseCase @Inject constructor(
    ...
    operator fun invoke(): Flow<Result<List<Usuario>>> {
        return flow {
            ...
            if (userId != null) {
                seguimientoRepository.getSeguidores(userId).collect { result ->
                    emit(result)
                }
            } else {
